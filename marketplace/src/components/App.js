...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if(networkData) {
      ...
      window.alert('Parduotuves sutartis nepaleista i tinkla')
    }
  }
  ...
  purchaseProduct = async (id, price) => {
    this.setState({ loading : true })
    await this.state.marketplace.methods.purchasedProduct(id).send({ from: this.state.account, value: price })
    .once('receipt', (receipt) => {
      this.setState({ loading: false })
    })
  }
