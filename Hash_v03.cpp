...
void string_pairing(string name, string &symbols, vector<string> &pair_1, vector<string> &pair_2, vector<string> &hash_pair_1, vector<string> &hash_pair_2) {
    char x;
...
void task_nr_4(string name, string symbols, vector<string> pair_1, vector<string> pair_2, vector<string> hash_pair_1, vector<string> hash_pair_2) {
    bool colision = false;
    int colisions = 0;
    for (int i = 0; i < hash_pair_1.size(); i++) {
        if (hash_pair_1[i] == hash_pair_2[i]) {
    ...
    if (colisions == false) {
        cout << "Kolizijos nera!" << endl;
    }
    else {
        cout << "Rastos " << colisions << " kolizijos" << endl;
    }
}

void task_nr_6(string name, string symbols, vector<string> pair_1, vector<string> pair_2, vector<string> hash_pair_1, vector<string> hash_pair_2) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 3) {
        ...
        if (name == "random_pairs_1_diff.txt") {
            task = true;
            string_pairing(name, symbols, pair_1, pair_2, hash_pair_1, hash_pair_2);
            task_nr_6(name, symbols, pair_1, pair_2, hash_pair_1, hash_pair_2);
        }
        else if (name == "random_pairs.txt") {
            task = true;
            string_pairing(name, symbols, pair_1, pair_2, hash_pair_1, hash_pair_2);
            task_nr_4(name, symbols, pair_1, pair_2, hash_pair_1, hash_pair_2);
        }
        else {
            char x;
            while (inFile.get(x)) {
